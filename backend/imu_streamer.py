...
class IMUStreamer:
    ...
    def _parse_data(self, data: bytes):
        ...
        try:
            ...
            self.latest_data = IMUData(roll=roll, pitch=pitch, yaw=yaw, timestamp=time.time())
    ...
    def calibrate(self):
        ...
        if self.latest_data:
            ...
            print(
                f"Calibrated: Roll={self.roll_offset:.2f}°, Pitch={self.pitch_offset:.2f}°, Yaw={self.yaw_offset:.2f}°"
            )
        else:
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            if data:
                print(
                    f"\rRoll: {data.roll:6.2f}° | Pitch: {data.pitch:6.2f}° | Yaw: {data.yaw:6.2f}°",
                    end="",
                )
                sys.stdout.flush()
