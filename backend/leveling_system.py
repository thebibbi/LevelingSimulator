...
class PlatformLevelingSystem:
    ...
    def __init__(self, platform_type: str = 'tripod', 
    ...
    def enable_auto_level(self, enable: bool = True):
        ...
        if enable and not self.running:
            self.running = True
            self.leveling_thread = threading.Thread(
                target=self._auto_level_loop,
                daemon=True
            )
        ...
        while self.running and self.auto_level_enabled:
            ...
            if self.leveling_enabled:
                ...
                if imu_data:
                    ...
                    orientation_change = np.linalg.norm(
                        current_orientation - self.last_orientation
                    )
    ...
    def get_status(self) -> dict:
        ...
        status = {
            ...
            'imu': {
                ...
                'tilt_magnitude': np.sqrt(imu_data.roll**2 + imu_data.pitch**2) if imu_data else None
            },
