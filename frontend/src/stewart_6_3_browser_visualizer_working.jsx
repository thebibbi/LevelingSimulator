...
function runSelfTests() {
  ...
  // Test 1: Rotation orthogonality
  {
    ...
    const I = matMul(Rt, R);
    const ok =
      Math.abs(I[0][0] - 1) < 1e-6 && Math.abs(I[1][1] - 1) < 1e-6 && Math.abs(I[2][2] - 1) < 1e-6;
  ...
  // Test 4: Pair symmetry at neutral pose (legs of each pair equal)
  {
    ...
    ok
      ? pass('Neutral pose: paired legs have equal lengths')
      : fail('Pair symmetry failed at neutral pose');
  }
      ...
      <div className="lg:col-span-1 space-y-4">
        ...
        <div className="rounded-2xl p-4 shadow bg-white/5 border border-white/10">
          ...
          <div className="text-sm opacity-80 mb-2">
            Base radius 120 mm, Platform radius 70 mm, leg pairs (0,1)-(2,3)-(4,5). Edit in code if
            you want exact anchors.
          </div>
        ...
        <div
          className={`rounded-2xl p-4 shadow border ${outOfRange ? 'border-red-500/40 bg-red-500/10' : 'border-white/10 bg-white/5'}`}
        >
        ...
        <div className="rounded-2xl p-4 shadow bg-white/5 border border-white/10">
          ...
          <div className="flex gap-3 text-sm">
            <span className="px-2 py-1 rounded-xl bg-white/10 border border-white/20">
              Passed: {tests.passed}
            </span>
            <span className="px-2 py-1 rounded-xl bg-white/10 border border-white/20">
              Failed: {tests.failed}
